...
int main()
{
  ...
  fstream f;
  f.open("/var/www/html/WIND/E.txt",ios::in);
  double xmin,xmax;
  while (!f.eof()) {
    f>>xmin>>xmax>>ws;
  }
  ...
  int iPrFlux60MeV = 0;
  string line;
  fileNM.open("/var/www/html/WIND/omni_27_av.dat"); // OMNIWeb DATA FILE
  fileNM2.open("/var/www/html/WIND/omni_27_av.dat"); // OMNIWeb DATA FILE
  ...
  outFile->cd();
    ...
    grWindSpeedVSTime->GetXaxis()->SetRangeUser(xmin,xmax);
