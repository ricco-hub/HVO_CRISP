...
with open("/var/www/html/WIND/E.txt", "r") as infile:
...
#   speed range                                      
ssnr = open("/var/www/html/WIND/data_PLOT/Speed_range.txt","w")
with open("/var/www/html/WIND/Speed.txt", "r") as infile:
...
# proton range
ssnr = open("/var/www/html/WIND/data_PLOT/Proton_range.txt","w")
with open("/var/www/html/WIND/Proton.txt", "r") as infile:
    SSN_all = infile.readlines()
    for i in range(len(SSN_all)):
     sline = SSN_all[i].split()
     if(  float(date[0])<= float(sline[0]) <= float(date[1])) :
