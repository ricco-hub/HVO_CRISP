...
for w in range(len(res)):
    for i in range(len(stat)):
        ...
        NM = (
            ...
            + stat[i]
            + "&tabchoice=1h&dtype=corr_for_efficiency&tresolution="
            + res[w]
            + "&yunits=0&date_choice=bydate&start_day=1&start_month=1&start_year=1960&start_hour=0&start_min=0&end_day=4&end_month=12&end_year=2019&end_hour=23&end_min=59&output=ascii"
        )
        year = str(now.year)
        month = str(now.month)
        day = str(now.day)
