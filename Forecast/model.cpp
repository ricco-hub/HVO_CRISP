...
{ 
  double Ekn;
  ifstream setE;
  string pathE ="/var/www/html/Forecast/E.txt";
  setE.open(pathE);
  string  sxE;
  while (!setE.eof()) {
    setE>>sxE>>ws;
  ...
  vector<double> JIS;
  vector<double> EIS;
  // importare la lista NM;

    vector<string> stats;
    
    ifstream set;
   
     string path ="/var/www/html/Forecast/NM_ALL.txt";
    cout<<path<<endl;
    ...
    for (int i=0; i<stats.size(); i++) {
      ...
      myfile.open("/var/www/html/Forecast/"+ stats[i]+"PHI.txt");
      for (int j=0; j<phi.size()-1; j++) {
	//	 cout<<"*********** "<<stats[i]<<endl;
	//	cout<<   std::setprecision(std::numeric_limits<double>::digits10)   <<date[j]<<" "<<phi[j]<<endl;
	myfile  <<  std::setprecision(std::numeric_limits<double>::digits10)        <<date[j]<<"  "<<phi[j]<<endl;
      }
	...
	// cout<<"********************"<<endl;	
	myfile2<<std::setprecision(std::numeric_limits<double>::digits10)   <<date[j]<<"  "<<JMOD[j]<<endl;
     
	//STAMPAAAA JMOd modulated
