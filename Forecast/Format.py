...
for i in range(len(stat)):
  ...
  file4.write("Modulation Potential values downloaded from HVO:\n Starting from "+stat[i]+" station monthly rate: \ncolumn 1 - Date in fraction of year.\ncolumn 2 - Modulation potential values [MV].\n\n")
...
for i in range(len(stat)):
  ...
  file4.write("Differential intensity (J) of cosmic ray (protons) near-Earth values downloaded from HVO:\n Starting from "+stat[i]+" station monthly rate: \ncolumn 1 - Date in fraction of year.\ncolumn 2 - J values [GeV^-1 m^-2 s^-1 sr^-1].\n\n")

  with open("/var/www/html/Forecast/data_PLOT/JMOD" + stat[i]+ ".txt", "r") as infile:
   SSN_all = infile.readlines()
   for i in range(len(SSN_all)):
    sline = SSN_all[i].split()
    print(sline)
