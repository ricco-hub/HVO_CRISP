...
with open(time.strftime("/var/www/html/SSN/SSN_Monthly.txt"), "w") as file:
    for j in range(len(monthly)):
        ...
        if (
            not sline[3] == "-1" or sline[3] == "-1.0"
        ):  # non stampo i valori non disponibili che SILSO indica con -1 o -1.0
...
with open(time.strftime("/var/www/html/SSN/SSN_13.txt"), "w") as outfile:
    for j in range(len(line)):
        ...
        if (
            sline[4] == "-1.0" or sline[4] == "-1"
        ):  # stampo 0 se non sono disponibili gli errori che SILSO indica con -1 o -1.0
            outfile.write("0" + " " + "0 " + "\n")
        else:
            outfile.write("0" + " " + sline[4] + " " + "\n")
...
with open(time.strftime("/var/www/html/SSN/SSN_Y.txt"), "w") as outfile3:
    for j in range(len(line)):
        ...
        if (
            sline[2] == "-1.0" or sline[2] == "-1"
        ):  # stampo 0 se non sono disponibili gli errori che SILSO indica con -1 o -1.0
            outfile3.write("0" + " " + "0" + "\n")
        else:
            outfile3.write("0" + " " + sline[2] + " " + "\n")
