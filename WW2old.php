...
//control error log
 for ($i = 0; $i < $result; $i++) {
 $txt = $stations[$i];
 if ($txt == "OULU" || $txt == "NEWK" || $txt == "APTY" || $txt == "JUNG" || $txt == "KIEL" || $txt == "MOSC" || $txt == "MXCO" || $txt == "ROME" ) {
   $err = 0; 
 }
else { $err = 1; }
}
...
else {echo "ERROR: Invalid Input";echo "<br>"; 
echo("<button onclick=\"location.href='/Intro2.html'\">Back</button>");}
