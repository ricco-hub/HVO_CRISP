...
//---------------------------------------Tilt Tree----------------------------------------------------
  ...
  //--------------------------------------TREE SSN SMOOTHED
   ...
   TFile *f5 =  new TFile("/var/www/html/TREE/ROOT/MonthlyTree.root","RECREATE");
    ...
    dato dJung,dNewk,dOulu,dKiel,dINVK,dKERG,dMOSC,dAPTY;
    ...
    TTree *tt7 = new TTree ("KERG","ex 1");
    tt7->Branch("KERG",&dKERG,"date/F:value");


    fstream KERG;
    KERG.open("/var/www/html/Neutron2/KERG.txt",ios::in);

    while (!KERG.eof()) {
      KERG>>dKERG.x>>dKERG.y>>ws;
      tt7->Fill();
    }
    KERG.close();
    tt7->Print();

    cout<<"KERG tree created"<<endl;
    
    //--------------------------------MOSC TREE--                                                           
                                                                                                            

    TTree *tt8 = new TTree ("MOSC","ex 1");
    tt8->Branch("MOSC",&dMOSC,"date/F:value");


    fstream MOSC;
    MOSC.open("/var/www/html/Neutron2/MOSC.txt",ios::in);

    while (!MOSC.eof()) {
      MOSC>>dMOSC.x>>dMOSC.y>>ws;
      tt8->Fill();
    }
    MOSC.close();
    tt8->Print();

    cout<<"MOSC tree created"<<endl;



    //--------------------------------APTY TREE--                                                                                               

    TTree *tt9 = new TTree ("APTY","ex 1");
    tt7->Branch("APTY",&dAPTY,"date/F:value");


    fstream APTY;
    APTY.open("/var/www/html/Neutron2/APTY.txt",ios::in);

    while (!APTY.eof()) {
      APTY>>dAPTY.x>>dAPTY.y>>ws;
      tt9->Fill();
    }
    APTY.close();
    tt9->Print();

    cout<<"APTY tree created"<<endl;


    //--------------------------------INVK TREE--                                                           \
                                                                                                             

    TTree *tt10 = new TTree ("INVK","ex 1");
    tt10->Branch("INVK",&dINVK,"date/F:value");


    fstream INVK;
    INVK.open("/var/www/html/Neutron2/INVK.txt",ios::in);

    while (!INVK.eof()) {
      INVK>>dINVK.x>>dINVK.y>>ws;
      tt10->Fill();
    }
    INVK.close();
    tt10->Print();

    cout<<"INVK tree created"<<endl;
