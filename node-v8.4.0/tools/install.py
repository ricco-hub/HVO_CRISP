...
def install(paths, dst):
    map(lambda path: try_copy(path, dst), paths)


def uninstall(paths, dst):
    map(lambda path: try_remove(path, dst), paths)
...
def npm_files(action):
    ...
    if os.path.islink(abspath(install_path, target_path)):
        return
...
def headers(action):
    ...
    if "true" == variables.get("node_use_openssl") and "false" == variables.get(
        "node_shared_openssl"
    ):
        subdir_files(
            "deps/openssl/openssl/include/openssl", "include/node/openssl/", action
        )
        subdir_files("deps/openssl/config/archs", "include/node/openssl/archs", action)
        action(["deps/openssl/config/opensslconf.h"], "include/node/openssl/")

    if "false" == variables.get("node_shared_zlib"):
        action(["deps/zlib/zconf.h", "deps/zlib/zlib.h",], "include/node/")
