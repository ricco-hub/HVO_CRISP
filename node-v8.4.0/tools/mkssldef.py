...
if __name__ == "__main__":
    ...
    while filenames and filenames[0].startswith("-"):
        ...
        if option == "-o":
            ...
            categories += option[2:].split(",")
        elif option.startswith("-D"):
            defines += option[2:].split(",")
        elif option.startswith("-X"):
            excludes += option[2:].split(",")
        elif option.startswith("-B"):
            bases += option[2:].split(",")
    ...
    for filename in filenames:
        for line in open(filename).readlines():
            ...
            if any(map(lambda p: p.match(name), excludes)):
                continue
            meta = meta.split(":")
            ...
            def satisfy(expr, rules):
                def test(expr):
                    if expr.startswith("!"):
                        return not expr[1:] in rules
                    return expr == "" or expr in rules

                return all(map(test, expr.split(",")))

            if not satisfy(meta[1], defines):
                continue
            if not satisfy(meta[3], categories):
                continue
