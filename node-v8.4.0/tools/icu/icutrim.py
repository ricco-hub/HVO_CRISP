...
parser = optparse.OptionParser(
    usage="usage: mkdir tmp ; %prog -D ~/Downloads/icudt53l.dat -T tmp -F trim_en.json -O icudt53l.dat"
)
...
parser.add_option(
    "-T", "--tmp-dir", action="store", dest="tmpdir", help="working directory.",
)  # required
...
parser.add_option(
    ...
    action="store",
    dest="endian",
    help='endian, big, little or host, your default is "%s".' % endian,
    default=endian,
    metavar="endianness",
)
...
for i in range(len(items)):
    ...
    if item.endswith(RES_INDX):
        ...
        runcmd(
            "iculslocs",
            "-i %s -N %s -T %s -l > %s" % (outfile, dataname, tree, treelistfile),
        )
        ...
        if not config.has_key("trees") or not config["trees"].has_key(tree):
            print " Warning: filter file %s does not mention trees.%s - will be kept as-is" % (
                options.filterfile,
                tree,
            )
        else:
...
def removeList(count=0):
    ...
    if len(remove) > 0:
        ...
        rc = runcmd(
            "icupkg", "-r %s %s 2> %s" % (removefile, outfile, hackerrfile), True
        )
        if rc is not 0:
            if options.verbose > 5:
...
for tree in trees:
    ...
    runcmd(
        "iculslocs", "-i %s -N %s -T %s -b %s" % (outfile, dataname, tree, treebundtxt)
    )
    runcmd("genrb", "-d %s -s %s res_index.txt" % (treebunddir, treebunddir))
    runcmd(
        "icupkg",
        "-s %s -a %s%s %s"
        % (options.tmpdir, trees[tree]["treeprefix"], RES_INDX, outfile),
    )
