...
def start_replay_server(args, sites, discard_output=True):
    with tempfile.NamedTemporaryFile(
        prefix="callstats-inject-", suffix=".js", mode="wt", delete=False
    ) as f:
        injection = f.name
...
def get_chrome_flags(js_flags, user_data_dir, arg_delimiter=""):
    return [
        ...
        "--no-first-run",
        "--user-data-dir={}{}{}".format(arg_delimiter, user_data_dir, arg_delimiter),
    ]
...
def get_chrome_replay_flags(args, arg_delimiter=""):
    ...
    return [
        ...
        "MAP *:443 localhost:%s, "
        "EXCLUDE localhost%s" % (arg_delimiter, http_port, https_port, arg_delimiter),
    ...
    while count == 0 or args.repeat is not None and count < args.repeat:
        ...
        while args.retries is None or retries < args.retries:
            ...
            try:
                ...
                cmd_args = (
                    ["timeout", str(timeout), args.with_chrome] + chrome_flags + [site]
                )
                print "- " * 40
                print_command(cmd_args)
                print "- " * 40
                ...
                print (
                    "EMPTY RESULT, REPEATING RUN ({})".format(retries_since_good_run)
                )
            finally:
...
def do_run_replay_server(args):
    ...
    flags = get_chrome_flags(
        "--runtime-call-stats --allow-natives-syntax", "/var/tmp/`date +%s`", '"'
    )
...
def read_stats(path, domain, args):
    ...
    with open(path, "rt") as f:
        ...
        group_data = {"time": 0, "count": 0}
        for group_name, regexp in groups:
            if not group_name.startswith("Group-Parse"):
                continue
            group_data["time"] += entries[group_name]["time"]
            group_data["count"] += entries[group_name]["count"]
        ...
        group_data = {"time": 0, "count": 0}
        for group_name, regexp in groups:
            if not group_name.startswith("Group-Compile"):
                continue
            group_data["time"] += entries[group_name]["time"]
            group_data["count"] += entries[group_name]["count"]
        ...
        for key in entries:
            if key not in domain:
                domain[key] = {"time_list": [], "count_list": []}
...
def print_stats(S, args):
    ...
    L = [
        item
        for item in sorted(S.items(), key=sort_func)
    ...
    def print_entry(key, value):
        ...
        print "{:>50s}  {}  {}".format(
            key, stats(value["time_stat"], units="ms"), stats(value["count_stat"])
        )
    ...
    for i in range(low, high):
        ...
        if args.totals and args.limit != 0 and not args.aggregate:
            if i == low:
                partial = {
                    "time_list": [0] * len(L[i][1]["time_list"]),
...
def do_stats(args):
    ...
    for i, domain in enumerate(sorted(domains)):
        ...
        for key in domain_stats:
            domain_stats[key]["time_stat"] = statistics(domain_stats[key]["time_list"])
            domain_stats[key]["count_stat"] = statistics(
                domain_stats[key]["count_list"]
            )
...
def main():
    ...
    subparsers["run"].set_defaults(func=do_run, error=subparsers["run"].error)
    subparsers["run"].add_argument(
        "--chrome-flags", type=str, default="", help="specify additional chrome flags"
    )
    subparsers["run"].add_argument(
        "--js-flags", type=str, default="", help="specify additional V8 flags"
    )
    subparsers["run"].add_argument(
        "-u",
        "--user-data-dir",
    ...
    subparsers["run"].add_argument(
        "--domain", type=str, default="", help="specify the output file domain name"
    )
    subparsers["run"].add_argument(
        "-n",
        "--repeat",
    ...
    subparsers["stats"].set_defaults(func=do_stats, error=subparsers["stats"].error)
    ...
    subparsers["json"].set_defaults(func=do_json, error=subparsers["json"].error)
    ...
    # Command: help.
    subparsers["help"] = subparser_adder.add_parser("help", help="help information")
