...
class GenerateGnArgs(object):
    ...
    def _parse_arguments(self, args):
        ...
        gen_cmd.add_argument("outdir", nargs="?", help="optional gn output directory")
        ...
        # Command: list.
        list_cmd = subps.add_parser("list", help="list available configurations")
    ...
    def cmd_gen(self):
        if not self._options.outdir and not self._options.builder:
            self.parser.error(
                "please specify either an output directory or "
        ...
        # goma off.
        modified |= self._append_gn_args("goma", gn_args_path, self._goma_args)
    ...
    def _find_work_dir(self, path):
        ...
        if os.path.exists(os.path.join(path, "tools", "dev", "v8gen.py")):
            ...
            raise Exception("This appears to not be called from a recent v8 checkout")
        else:
