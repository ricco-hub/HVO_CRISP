...
def GenerateTests(options):
    ...
    output = subprocess.check_output(
        "%s %s" % (options.d8, options.script), shell=True
    ).strip()
...
def BuildOptions():
    ...
    result.add_option("--d8", help="d8 binary to use", default="out/ia32.release/d8")
    result.add_option(
        "--outdir",
        help="directory where to place generated tests",
        default="test/mjsunit/builtins-gen",
    )
    result.add_option(
        "--script",
        help="builtins detector script to run in d8",
