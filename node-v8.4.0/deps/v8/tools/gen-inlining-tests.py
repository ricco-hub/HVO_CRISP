...
def fnname(flags):
    ...
    return "f_" + "".join(FLAGLETTERS[i] if b else "_" for (i, b) in enumerate(flags))
...
def printtest(flags):
    ...
    if not (tryReturns or tryThrows):
        return

    # Either doCatch or doFinally.
    if not doCatch and not doFinally:
        return
    ...
    # true.
    if tryFirstReturns and not (tryReturns and tryThrows):
        return

    # From the try and finally block, we can return or throw, but not both.
    if catchReturns and catchThrows:
        return
    if finallyReturns and finallyThrows:
        return

    # If at the end we return the local, we need to have touched it.
    if endReturnLocal and not (tryResultToLocal or catchWithLocal):
        return

    # PRUNING

    anyAlternative = any(
        [alternativeFn1, alternativeFn2, alternativeFn3, alternativeFn4, alternativeFn5]
    )
    ...
    if (
        tryReturns
        and tryThrows
        and (catchWithLocal or endReturnLocal or deopt or anyAlternative)
    ):
        return
    # We don't do any alternative if we do a finally.
    if doFinally and anyAlternative:
        return
    # We only use the local variable if we do alternative #2 or #3.
    if (
        tryResultToLocal or catchWithLocal or endReturnLocal
    ) and not specificAlternative:
        return
    # We don't need to test deopting into a finally.
    if doFinally and deopt:
        return
...
allFlagCombinations = [
    flagtuple(*bools) for bools in booltuples(len(flagtuple._fields))
]
