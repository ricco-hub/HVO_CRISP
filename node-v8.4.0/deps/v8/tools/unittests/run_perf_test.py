...
V8_JSON = {
    ...
    "run_count": 1,
    "results_regexp": "^%s: (.+)$",
    "tests": [{"name": "Richards"}, {"name": "DeltaBlue"},],
}
...
V8_NESTED_SUITES_JSON = {
    ...
    "tests": [
        ...
        {"name": "ShouldntRun", "path": ["."], "archs": ["arm"], "main": "run.js"},
    ],
}
...
class PerfTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls._cov = coverage.coverage(include=([os.path.join(cls.base, "run_perf.py")]))
        cls._cov.start()
        import run_perf
    ...
    def _MockCommand(self, *args, **kwargs):
        ...
        test_outputs = [
            Output(stdout=arg, stderr=None, timed_out=kwargs.get("timed_out", False))
            for arg in args[1]
        ]
    ...
    def _VerifyMock(self, binary, *args, **kwargs):
        ...
        arg += args
        commands.Execute.assert_called_with(arg, timeout=kwargs.get("timeout", 60))
    ...
    def testAndroid(self):
        ...
        self.assertEquals(
            0, self._CallMain("--android-build-tools", "/some/dir", "--arch", "arm")
        )
        self._VerifyResults(
            "test",
            "score",
    ...
    def testWrongBinaryWithProf(self):
        ...
        self._VerifyErrors([])
        self._VerifyMock(
            path.join("out", "x64.release", "d7"), "--flag", "--prof", "run.js"
        )
    ...
    def _RunPerf(self, mocked_d8, test_json):
        ...
        args = [
            ...
            os.path.join(TEST_DATA, mocked_d8),
            "--json-test-results",
            output_json,
