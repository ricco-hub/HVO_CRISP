    ...
    with open(patch_file) as patch:
        for line in patch:
            if line.startswith("diff --git "):
                ...
                if old_hash.startswith("0000000"):
                    continue  # Ignore new files.
...
def FindFirstMatchingCommit(start, files, limit, verbose):
    ...
    num_files = len(files)
    if verbose:
        print(">>> Found %d files modified by patch." % num_files)
    for _ in range(limit):
        matched_files = CountMatchingFiles(commit, files)
        if verbose:
            print("Commit %s matched %d files" % (commit, matched_files))
        if matched_files == num_files:
            return commit
        commit = GetGitCommitHash("%s^" % commit)
    print(
        "Sorry, no matching commit found. "
