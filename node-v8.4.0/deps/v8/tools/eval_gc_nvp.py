    ...
    def __str__(self):
        ...
        for i in range(0, last + 1):
            ...
            if i == keys[0]:
                ...
                if self.fill_empty:
                    ret.append(
                        "  [{0},{1}[: {2}".format(str(min_value), str(max_value), 0)
                    )
        return "\n".join(ret)
...
def main():
    ...
    parser.add_argument(
        "keys", metavar="KEY", type=str, nargs="+", help="the keys of NVPs to process"
    )
    parser.add_argument(
        "--histogram-type",
        metavar="<linear|log2>",
    ...
    log2_group.add_argument(
        "--log2-histogram-init-bucket",
        metavar="START",
        type=int,
        nargs="?",
        default=64,
    ...
    parser.add_argument(
        "--csv", dest="csv", action="store_true", help="provide output as csv"
    )
    parser.add_argument(
        "--percentiles",
        dest="percentiles",
    ...
    categories = [
        Category(key, deepcopy(histogram), args.csv, percentiles) for key in args.keys
    ]
