...
class RetrieveV8Releases(Step):
    ...
    def GetReleaseDict(
        self, git_hash, master_position, master_hash, branch, version, patches, cl_body
    ):
        ...
        return {
            ...
            "revision_link": (
                "https://code.google.com/p/v8/source/detail?r=%s" % revision
            ),
        }
    ...
    def GetRelease(self, git_hash, branch):
        ...
        return (
            self.GetReleaseDict(
                git_hash, master_position, master_hash, branch, version, patches, body
            ),
            self["patch"],
        )
    ...
    def GetReleaseFromRevision(self, revision):
        releases = []
        try:
            if VERSION_FILE not in self.GitChangedFiles(
                revision
            ) or not self.GitCheckoutFileSafe(VERSION_FILE, revision):
    ...
    def RunStep(self):
        ...
        if self._options.branch == "recent":
            ...
            assert self._options.branch in (
                self.vc.GetBranches()
        ...
        self["releases"] = sorted(
            releases, key=lambda r: SortingKey(r["version"]), reverse=True
        )
    ...
    def RunStep(self):
        ...
        try:
            for git_hash in self.GitLog(
                format="%H", grep="V8", branch="origin/master", path="DEPS", cwd=cwd
            ).splitlines():
...
class RetrieveChromiumBranches(Step):
    ...
    def RunStep(self):
        ...
        branches = filter(
            lambda r: re.match(r"branch-heads/\d+", r), self.GitRemotes(cwd=cwd)
        )

        # Transform into pure branch numbers.
        branches = map(
            lambda r: int(re.match(r"branch-heads/(\d+)", r).group(1)), branches
        )
...
class RetrieveInformationOnChromeReleases(Step):
    ...
    def Run(self):
        ...
        result_raw = self.ReadURL(
            OMAHA_PROXY_URL + "all.json", params, wait_plan=[5, 20]
        )
    ...
    def _CreateCandidate(self, current_version):
        params = None
        url_to_call = (
            OMAHA_PROXY_URL + "v8.json?version=" + current_version["previous_version"]
        )
        result_raw = self.ReadURL(url_to_call, params, wait_plan=[5, 20])
...
class Releases(ScriptsBase):
    def _PrepareOptions(self, parser):
        parser.add_argument(
            ...
            default="recent",
            help=(
                "The branch to analyze. If 'all' is specified, "
        ...
        parser.add_argument(
            ...
            "--chromium",
            help=(
                "The path to your Chromium src/ " "directory to automate the V8 roll."
            ),
        )
