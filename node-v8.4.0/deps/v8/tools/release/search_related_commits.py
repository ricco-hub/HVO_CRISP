...
def _search_related_commits(
    ...
    original_message = git_execute(
        git_working_dir, ["show", "-s", "--format=%B", start_hash], verbose
    )
    ...
    found_by_hash = git_execute(git_working_dir, git_args(start_hash), verbose).strip()

    if verbose:
        print "2.) Found by hash: " + found_by_hash

    found_by_commit_pos = git_execute(
        git_working_dir, git_args(commit_position), verbose
    ).strip()
    ...
    found_by_title = git_execute(git_working_dir, git_args(title), verbose).strip()
...
def _pretty_print_entry(hash, git_dir, pre_text, verbose):
    text_to_print = git_execute(
        git_dir,
        ["show", "--quiet", "--date=iso", hash, "--format=%ad # %H # %s"],
        verbose,
    )
...
if __name__ == "__main__":  # pragma: no cover
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Enables a very verbose output"
    )
    parser.add_argument("of", nargs=1, help="Hash of the commit to be searched.")
    parser.add_argument("until", nargs=1, help="Commit when searching should stop")
    parser.add_argument(
        "--separator",
        required=False,
    ...
    parser.add_argument(
        "--prettyprint", action="store_true", help="Pretty prints the output"
    )
