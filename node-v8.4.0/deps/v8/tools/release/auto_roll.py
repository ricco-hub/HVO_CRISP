...
class DetectRevisionToRoll(Step):
    ...
    def RunStep(self):
        ...
        for revision in revisions:
            ...
            print (
                "There is no newer v8 revision than the one in Chromium (%s)."
                % self["last_roll"]
            )
...
class PrepareRollCandidate(Step):
    ...
    def RunStep(self):
        ...
        self["roll_title"] = self.GitLog(n=1, format="%s", git_hash=self["roll"])
...
class UploadCL(Step):
    ...
    def RunStep(self):
        ...
        cwd = self._options.chromium
        # Patch DEPS file.
        if self.Command("roll-dep-svn", "v8 %s" % self["roll"], cwd=cwd) is None:
            self.Die("Failed to create deps for %s" % self["roll"])

        message = []
        message.append("Update V8 to %s." % self["roll_title"].lower())

        message.append(ROLL_SUMMARY % (self["last_roll"][:8], self["roll"][:8]))
...
class CleanUp(Step):
    ...
    def RunStep(self):
        ...
        print (
            "Congratulations, you have successfully rolled %s into "
            "Chromium." % self["roll"]
        )
...
class AutoRoll(ScriptsBase):
    def _PrepareOptions(self, parser):
        ...
        parser.add_argument(
            "--revision", help="Revision to roll. Auto-detected if not " "specified."
        ),
        parser.add_argument(
            "--roll", help="Deprecated.", default=True, action="store_true"
        )
        parser.add_argument(
            "--use-commit-queue",
