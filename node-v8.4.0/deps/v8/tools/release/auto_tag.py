...
class GetOldestUntaggedVersion(Step):
    ...
    def RunStep(self):
        ...
        for git_hash in self.GitLog(
            format="%H", grep="\\[Auto\\-roll\\] Bump up version to"
        ).splitlines():
            ...
            if version in tags:
                if self["candidate"]:
                    ...
                    print (
                        "Stop as %s is the latest version and it has been tagged."
                        % version
                    )
                    self.CommonCleanup()
                    return True
        ...
        print (
            "Candidate for tagging is %s with version %s"
...
class GetLKGRs(Step):
    ...
    def RunStep(self):
        ...
        self["lkgrs"] = [
            entry["revision"] for entry in json.loads(status_json) if entry["status"]
        ]
...
class MakeTag(Step):
    ...
    def RunStep(self):
        if not self._options.dry_run:
            ...
            self.vc.Tag(
                self["candidate_version"], "svn/bleeding_edge", "This won't work!"
            )
...
class AutoTag(ScriptsBase):
    def _PrepareOptions(self, parser):
        parser.add_argument(
            "--dry_run",
            help="Don't tag the new version.",
            default=False,
            action="store_true",
        )
