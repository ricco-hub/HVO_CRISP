...
class PrepareBranchRevision(Step):
    ...
    def RunStep(self):
        self["push_hash"] = self._options.revision or self.GitLog(
            n=1, format="%H", branch="origin/master"
        )
...
class EnableMergeWatchlist(Step):
    ...
    def RunStep(self):
        old_watchlist_content = FileToText(
            os.path.join(self.default_cwd, WATCHLISTS_FILE)
        )
        new_watchlist_content = re.sub(
            "(# 'v8-merges@googlegroups\.com',)",
            "'v8-merges@googlegroups.com',",
            old_watchlist_content,
        )
        TextToFile(
            new_watchlist_content, os.path.join(self.default_cwd, WATCHLISTS_FILE)
        )
...
class TagRevision(Step):
    ...
    def RunStep(self):
        if self._options.dry_run:
            print (
                'Dry run. Tagging "%s" with %s'
                % (self["commit_title"], self["version"])
            )
        else:
            self.vc.Tag(
                self["version"], "origin/%s" % self["version"], self["commit_title"]
            )
...
class CleanUp(Step):
    ...
    def RunStep(self):
        print (
            "Congratulations, you have successfully created version %s."
            % self["version"]
        )
...
class CreateRelease(ScriptsBase):
    ...
    def _Config(self):
        return {
            ...
            "CHANGELOG_ENTRY_FILE": "/tmp/v8-create-releases-tempfile-changelog-entry",
