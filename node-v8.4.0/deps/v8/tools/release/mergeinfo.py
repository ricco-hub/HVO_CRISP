...
def describe_commit(git_working_dir, hash_to_search, one_line=False):
    if one_line:
        return git_execute(
            git_working_dir,
            ["show", GIT_OPTION_NO_DIFF, GIT_OPTION_ONELINE, hash_to_search],
        ).strip()
    return git_execute(
        git_working_dir, ["show", GIT_OPTION_NO_DIFF, hash_to_search]
    ).strip()
...
def get_merge_commits(git_working_dir, hash_to_search):
    ...
    return [merge_commit for merge_commit in merges if merge_commit not in false_merges]
...
def get_branches_for_commit(git_working_dir, hash_to_search):
    branches = git_execute(
        git_working_dir, ["branch", "--contains", hash_to_search, "-a"]
    ).strip()
...
def get_first_canary(branches):
    canaries = [
        ...
        if currentBranch.startswith("remotes/origin/chromium/")
    ]
    canaries.sort()
    if len(canaries) == 0:
...
def print_analysis(git_working_dir, hash_to_search):
    ...
    print "First V8 branch: " + str(
        get_first_v8_version(branches)
    ...
    for currentMerge in merges:
        ...
        print "---Merged to:"
        mergeOutput = git_execute(
            git_working_dir, ["branch", "--contains", currentMerge, "-r"]
        ).strip()
        print mergeOutput
...
if __name__ == "__main__":  # pragma: no cover
    parser = argparse.ArgumentParser(
        "Tool to check where a git commit was" " merged and reverted."
    )
