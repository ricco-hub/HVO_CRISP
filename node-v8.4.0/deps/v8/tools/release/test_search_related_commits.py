...
class TestSearchRelatedCommits(unittest.TestCase):
    ...
    def _execute_git(self, git_args):
        ...
        p = Popen(args=fullCommand, stdin=PIPE, stdout=PIPE, stderr=PIPE)
    ...
    def _assert_correct_standard_result(
        ...
        self.assertTrue(result.get(hash_of_first_commit), "Master commit is wrong")

        self.assertEqual(len(result[hash_of_first_commit]), 1, "Child commit not found")
        self.assertEqual(
            all_commits[2], result[hash_of_first_commit][0], "Child commit wrong"
        )

    def _get_commits(self):
        commits = self._execute_git(["log", "--format=%H", "--reverse"]).splitlines()
        return commits
    ...
    def testConsiderSeparator(self):
        ...
        message = (
            """Revert of some stuff: Not a hit
    > [turbofan] Sanitize language mode for javascript operators.
    > Reverting """
            + hash_of_first_commit
            + """
    ...
    Cr-Commit-Position: refs/heads/master@{#28088}"""
        ...
        message = (
            """Implement awesome feature: Master commit
    ...
    Cr-Commit-Position: refs/heads/master@{#29567}"""
        ...
        self.assertEqual(
            len(result.get(hash_of_hit)), 1, "More than one child hit found"
        )
        self.assertEqual(
            result.get(hash_of_hit)[0], hash_of_child_hit, "Wrong commit found"
        )
    ...
    Cr-Commit-Position: refs/heads/master@{#28088}"""
        ...
        OptionsStruct = namedtuple(
            "OptionsStruct", "git_dir of until all prettyprint separator verbose"
        )
