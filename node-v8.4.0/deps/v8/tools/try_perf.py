...
def main():
    ...
    parser.add_argument(
        "--extra-flags", default="", help="Extra flags to be passed to the executable."
    )
    parser.add_argument(
        "-r",
        "--revision",
    ...
    for benchmark in options.benchmarks:
        if benchmark not in PUBLIC_BENCHMARKS:
            print (
                "%s not found in our benchmark list. The respective trybot might "
    ...
    assert (
        '"' not in options.extra_flags and "'" not in options.extra_flags
