...
CONFIGS = dict(
    default=[],
    fullcode=["--noopt", "--turbo-filter=~",],
    ignition=["--ignition", "--turbo-filter=~", "--hydrogen-filter=~", "--noopt",],
    ignition_asm=[
        "--ignition",
        "--turbo-filter=~",
        "--hydrogen-filter=~",
    ...
    ignition_turbo=["--ignition", "--turbo",],
    ignition_turbo_opt=["--ignition", "--turbo", "--always-opt",],
...
def parse_args():
    ...
    parser.add_argument(
        "--random-seed", type=int, required=True, help="random seed passed to both runs"
    )
    parser.add_argument(
    ...
    assert os.path.exists(options.testcase) and os.path.isfile(options.testcase), (
    ...
    assert (
        options.first_arch != options.second_arch
        or options.first_config != options.second_config
...
def content_bailout(content, ignore_fun):
    ...
    if bug:
        print FAILURE_HEADER_TEMPLATE % dict(configs="", source_key="", suppression=bug)
        return True
    return False
...
def main():
    ...
    def run_d8(d8, config_flags):
        ...
        return v8_commands.Execute(
            args, cwd=os.path.dirname(options.testcase), timeout=TIMEOUT,
        )
