...
IGNORE_SOURCES = {
    ...
    "crbug.com/688159": ["/v8/test/mjsunit/es7/exponentiation-operator.js",],
    # TODO(machenbach): Implement blacklisting files for particular configs only,
    # here ignition_eager.
    "crbug.com/691589": ["/v8/test/mjsunit/regress/regress-1200351.js",],
    "crbug.com/691587": ["/v8/test/mjsunit/asm/regress-674089.js",],
}
...
IGNORE_OUTPUT = {
    "": {
        "crbug.com/664068": re.compile(r"RangeError(?!: byte length)", re.S),
        "crbug.com/667678": re.compile(r"\[native code\]", re.S),
        "crbug.com/689877": re.compile(r"^.*SyntaxError: .*Stack overflow$", re.M),
    },
}
...
def caret_match(line1, line2):
    if (
        not line1
        or not line2
        or len(line1) > MAX_LINE_LENGTH
        or len(line2) > MAX_LINE_LENGTH
    ):
        return False
...
class V8Suppression(Suppression):
    ...
    def ignore_by_content(self, testcase):
        ...
        try:
            ...
            lines = lines[
                lines.index('print("js-mutation: start generated test case");') :
            ]
            content = "\n".join(lines)
        except ValueError:
