...
class UnitTest(unittest.TestCase):
    ...
    def test_clobber(self):
        ...
        self.check(["gen", "-c", "gyp_debug", "//out/Debug"], mbw=mbw, ret=0)
        self.assertEqual(mbw.rmdirs, ["/fake_src/out/Debug", "/fake_src/out/Debug"])
        self.assertEqual(mbw.files["/fake_src/out/Debug/mb_type"], "gyp")
    ...
    def test_gn_gen(self):
        mbw = self.fake_mbw()
        self.check(
            ["gen", "-c", "gn_debug_goma", "//out/Default", "-g", "/goma"],
            mbw=mbw,
            ret=0,
        )
        ...
        self.assertIn(
            "/fake_src/buildtools/linux64/gn gen //out/Default --check", mbw.out
        )

        mbw = self.fake_mbw(win32=True)
        self.check(
            ["gen", "-c", "gn_debug_goma", "-g", "c:\\goma", "//out/Debug"],
            mbw=mbw,
            ret=0,
        )
        self.assertMultiLineEqual(
        ...
        self.assertIn(
            "c:\\fake_src\\buildtools\\win\\gn.exe gen //out/Debug " "--check\n",
            mbw.out,
        )

        mbw = self.fake_mbw()
        self.check(
            ["gen", "-m", "fake_master", "-b", "fake_gn_args_bot", "//out/Debug"],
            mbw=mbw,
            ret=0,
        )
        self.assertEqual(
    ...
    def test_gn_isolate(self):
        ...
        self.check(
            ["isolate", "-c", "gn_debug_goma", "//out/Default", "base_unittests"],
            files=files,
            ret=0,
        )
        ...
        self.check(["isolate", "//out/Default", "base_unittests"], files=files, ret=0)
    ...
    def test_gn_run(self):
        ...
        self.check(
            ["run", "-c", "gn_debug_goma", "//out/Default", "base_unittests"],
            files=files,
            ret=0,
        )
    ...
    def test_gyp_crosscompile(self):
        mbw = self.fake_mbw()
        self.check(["gen", "-c", "gyp_crosscompile", "//out/Release"], mbw=mbw, ret=0)
        self.assertTrue(mbw.cross_compile)

    def test_gyp_gen(self):
        self.check(
            ["gen", "-c", "gyp_rel_bot", "-g", "/goma", "//out/Release"],
            ret=0,
            out=(
                "GYP_DEFINES='goma=1 gomadir=/goma'\n"
        ...
        mbw = self.fake_mbw(win32=True)
        self.check(
            ["gen", "-c", "gyp_rel_bot", "-g", "c:\\goma", "//out/Release"],
            mbw=mbw,
            ret=0,
            out=(
                "set GYP_DEFINES=goma=1 gomadir='c:\\goma'\n"
                "python build\\gyp_chromium -G output_dir=out\n"
            ),
        )
    ...
    def test_gyp_lookup_goma_dir_expansion(self):
        self.check(
            ["lookup", "-c", "gyp_rel_bot", "-g", "/foo"],
            ret=0,
            out=(
                "GYP_DEFINES='goma=1 gomadir=/foo'\n"
    ...
    def test_multiple_phases(self):
        ...
        mbw = self.check(
            ["lookup", "-m", "fake_master", "-b", "fake_multi_phase"], ret=1
        )
        self.assertIn("Must specify a build --phase", mbw.out)

        # Check that passing a --phase to a single-phase builder fails.
        mbw = self.check(
            ["lookup", "-m", "fake_master", "-b", "fake_gn_builder", "--phase", "1"],
            ret=1,
        )
        ...
        mbw = self.check(
            ["lookup", "-m", "fake_master", "-b", "fake_multi_phase", "--phase", "0"],
            ret=1,
        )
        self.assertIn("Phase 0 out of bounds", mbw.out)

        mbw = self.check(
            ["lookup", "-m", "fake_master", "-b", "fake_multi_phase", "--phase", "1"],
            ret=0,
        )
        self.assertIn("phase = 1", mbw.out)

        mbw = self.check(
            ["lookup", "-m", "fake_master", "-b", "fake_multi_phase", "--phase", "2"],
            ret=0,
        )
        self.assertIn("phase = 2", mbw.out)

        mbw = self.check(
            ["lookup", "-m", "fake_master", "-b", "fake_multi_phase", "--phase", "3"],
            ret=1,
        )
    ...
    def test_gyp_env_hacks(self):
        ...
        self.check(
            ["lookup", "-c", "fake_config"],
            mbw=mbw,
            ret=0,
            out=(
                "GYP_DEFINES='foo=bar baz=1'\n"
