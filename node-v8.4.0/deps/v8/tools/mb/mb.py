...
class MetaBuildWrapper(object):
    def __init__(self):
        ...
        self.default_config = os.path.join(
            self.chromium_src_dir, "infra", "mb", "mb_config.pyl"
        )
    ...
    def ParseArgs(self, argv):
        def AddCommonOptions(subp):
            ...
            subp.add_argument(
                "--phase",
                type=int,
                help=("build phase for a given build " "(int in [1, 2, ...))"),
            )
            subp.add_argument(
                "-f",
                "--config-file",
                ...
                help="path to config file " "(default is //tools/mb/mb_config.pyl)",
            )
        ...
        subp.add_argument(
            ...
            help="save runtime dependencies for targets listed " "in file.",
        )
        subp.add_argument("path", nargs=1, help="path to generate build into")
        subp.set_defaults(func=self.CmdGen)

        subp = subps.add_parser(
            "isolate", help="generate the .isolate files for a given" "binary"
        )
        AddCommonOptions(subp)
        subp.add_argument("path", nargs=1, help="path build was generated into")
        subp.add_argument(
            "target", nargs=1, help="ninja target to generate the isolate for"
        )
        subp.set_defaults(func=self.CmdIsolate)

        subp = subps.add_parser(
            "lookup", help="look up the command for a given config or " "builder"
        )
        AddCommonOptions(subp)
        ...
        subp = subps.add_parser(
            "run",
            help="build and run the isolated version of a " "binary",
        ...
        subp.add_argument(
            ...
            metavar="PATH",
            default=self.default_config,
            help="path to config file " "(default is //infra/mb/mb_config.pyl)",
        )
        subp.set_defaults(func=self.CmdValidate)

        subp = subps.add_parser("audit", help="Audit the config file to track progress")
        subp.add_argument(
            "-f",
            "--config-file",
            metavar="PATH",
        ...
        subp.add_argument(
            "-i", "--internal", action="store_true", help="check internal masters also"
        )
        subp.add_argument(
            "-m",
            "--master",
            action="append",
            help="master to audit (default is all non-internal " "masters in file)",
        )
        subp.add_argument(
            "-u",
            "--url-template",
            action="store",
            default="https://build.chromium.org/p/" "{master}/json/builders",
            help="URL scheme for JSON APIs to buildbot " "(default: %(default)s) ",
        )
        subp.add_argument(
            "-c",
            "--check-compile",
            action="store_true",
            help="check whether tbd and master-only bots actually" " do compiles",
        )
    ...
    def DumpInputFiles(self):
        ...
        if getattr(self.args, "input_path", None):
            DumpContentsOfFilePassedTo("argv[0] (input_path)", self.args.input_path[0])
    ...
    def CmdValidate(self, print_ok=True):
        ...
        for config, mixins in self.configs.items():
            for mixin in mixins:
                if not mixin in self.mixins:
                    errs.append(
                        'Unknown mixin "%s" referenced by config "%s".'
                        % (mixin, config)
                    )
        ...
        for mixin in self.mixins:
            for sub_mixin in self.mixins[mixin].get("mixins", []):
                if not sub_mixin in self.mixins:
                    errs.append(
                        'Unknown mixin "%s" referenced by mixin "%s".'
                        % (sub_mixin, mixin)
                    )
        ...
        if errs:
            raise MBErr(
                ("mb config file %s has problems:" % self.args.config_file)
                + "\n  "
                + "\n  ".join(errs)
            )
    ...
    def ReadConfigFile(self):
        ...
        try:
            ...
            raise MBErr(
                'Failed to parse config file "%s": %s' % (self.args.config_file, e)
            )
    ...
    def ConfigFromArgs(self):
        if self.args.config:
            if self.args.master or self.args.builder:
                raise MBErr(
                    "Can not specific both -c/--config and -m/--master or "
                    "-b/--builder"
                )
        ...
        if isinstance(config, list):
            ...
            if phase < 1 or phase > len(config):
                raise MBErr(
                    "Phase %d out of bounds for %s on %s"
        ...
        if self.Exists(build_dir):
            if self.Exists(mb_type_path):
                ...
                if old_mb_type != new_mb_type:
                    self.Print(
                        "Build type mismatch: was %s, will be %s, clobbering %s"
    ...
    def RunGNGen(self, vals):
        ...
        if getattr(self.args, "swarming_targets_file", None):
            ...
            gn_isolate_map = ast.literal_eval(
                self.ReadFile(
                    self.PathJoin(
                        self.chromium_src_dir,
                        "testing",
            ...
            if err:
                raise MBErr(
                    "Error: Failed to match swarming targets to %s:\n%s"
        ...
        for target in swarming_targets:
            if android:
                ...
                gn_isolate_map[target]["type"] == "script"
                or gn_isolate_map[target].get("label_type") == "group"
            ):
            ...
            for r in runtime_deps_targets:
                ...
                raise MBErr(
                    "did not generate any of %s" % ", ".join(runtime_deps_targets)
                )
            ...
            self.WriteIsolateFiles(
                build_dir, command, target, runtime_deps, extra_files
            )
    ...
    def RunGNIsolate(self, vals):
        ...
        self.WriteIsolateFiles(build_dir, command, target, runtime_deps, extra_files)
    ...
    def WriteIsolateFiles(self, build_dir, command, target, runtime_deps, extra_files):
    ...
    def GNArgs(self, vals):
        if vals["cros_passthrough"]:
            ...
            if not re.search('target_os.*=.*"chromeos"', gn_args):
                raise MBErr(
                    'GN_ARGS is missing target_os = "chromeos": (GN_ARGS=%s)' % gn_args
                )
        else:
    ...
    def RunGYPAnalyze(self, vals):
        ...
        cmd.extend(
            [
                ...
                "analyzer",
                "-G",
                "config_path=%s" % self.args.input_path[0],
                "-G",
                "analyzer_output_path=%s" % self.args.output_path[0],
            ]
        )
        ret, _, _ = self.Run(cmd, env=env)
    ...
    def GetIsolateCommand(self, target, vals, gn_isolate_map):
        ...
        use_x11 = (
            ...
            and not android
            and not "use_ozone=true" in vals["gn_args"]
        )
        ...
        if android and test_type != "script":
            logdog_command = [
                ...
                "--prefix",
                "android/swarming/logcats/${SWARMING_TASK_ID}",
                "--source",
    ...
    def GYPCmd(self, output_dir, vals):
        if vals["cros_passthrough"]:
            ...
            if not "chromeos=1" in gyp_defines:
                raise MBErr(
                    "GYP_DEFINES is missing chromeos=1: (GYP_DEFINES=%s)" % gyp_defines
                )
        else:
    ...
    def RunGNAnalyze(self, vals):
        ...
        targets = set(inp["test_targets"]) | set(inp["additional_compile_targets"])
        ...
        if (
            any(f.endswith(".gn") or f.endswith(".gni") for f in inp["files"])
            or "all" in targets
        ):
            self.WriteJSON(
                {
        ...
        try:
            ...
            if ret and not "The input matches no targets" in out:
                self.WriteFailureAndRaise(
                    "gn refs returned %d: %s" % (ret, out), output_path
                )
            ...
            cmd = self.GNCmd(
                "refs", self.args.path[0], "@%s" % response_file.name, "--all"
            )
            ret, out, _ = self.Run(cmd, force_verbose=False)
            if ret and not "The input matches no targets" in out:
                self.WriteFailureAndRaise(
            ...
            for label in out.splitlines():
                ...
                for input_target in targets:
                    if input_target == build_target or build_target.endswith(
                        ":" + input_target
                    ):
            ...
            self.WriteJSON(
                {
                    ...
                    "test_targets": sorted(matching_targets & set(inp["test_targets"])),
                },
                output_path,
            )
        else:
    ...
    def CheckCompile(self, master, builder):
        ...
        url = urllib2.quote(
            url_template.format(master=master, builder=builder), safe=":/()?="
        )
        try:
        ...
        successes = sorted(
            [
                int(x)
                for x in builds.keys()
                if "text" in builds[x]
                and cmp(builds[x]["text"][:2], ["build", "successful"]) == 0
            ],
            reverse=True,
        )
        if not successes:
    ...
    def Call(self, cmd, env=None, buffer_output=True):
        if buffer_output:
            ...
            p = subprocess.Popen(cmd, shell=False, cwd=self.chromium_src_dir, env=env)
            p.wait()
            out = err = ""
