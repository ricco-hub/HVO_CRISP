...
def print_disassembly_annotation(offset_counts, bytecode_disassembly):
    ...
    if offsets:
        print (
            "WARNING: Offsets not empty. Output is most likely invalid due to "
...
def parse_command_line():
    ...
    command_line_parser.add_argument(
        "--arch", "-a", help="The architecture (default: x64)", default="x64",
    )
    command_line_parser.add_argument(
        "--input",
        "-i",
...
def main():
    ...
    perf = subprocess.Popen(
        ["perf", "script", "-f", "ip,sym,symoff", "-i", program_options.perf_filename],
        stdout=subprocess.PIPE,
    )

    v8_root_path = os.path.dirname(__file__) + "/../../"
    d8_path = "{}/out/{}.debug/d8".format(v8_root_path, program_options.arch)
    d8_codegen = subprocess.Popen(
        [d8_path, "--ignition", "--trace-ignition-codegen", "-e", "1"],
        stdout=subprocess.PIPE,
    )
