    ...
    for source, counters_from_source in iteritems(dispatches_table):
        for destination, counter in iteritems(counters_from_source):
            if counter == __COUNTER_MAX:
                print "WARNING: {} -> {} may have saturated.".format(
                    source, destination
                )
...
def find_top_bytecode_dispatch_pairs(dispatches_table, top_count):
    ...
    return heapq.nlargest(top_count, flattened_counters_generator(), key=lambda x: x[2])


def print_top_bytecode_dispatch_pairs(dispatches_table, top_count):
    top_bytecode_dispatch_pairs = find_top_bytecode_dispatch_pairs(
        dispatches_table, top_count
    )
...
def find_top_dispatch_sources_and_destinations(
    ...
    return (
        heapq.nlargest(
            top_count, sources, key=lambda x: x[2 if sort_source_relative else 1]
        ),
        heapq.nlargest(top_count, destinations, key=lambda x: x[1]),
    )


def print_top_dispatch_sources_and_destinations(
...
def plot_dispatches_table(dispatches_table, figure, axis):
    ...
    axis.xaxis.set(ticks=numpy.arange(0.5, len(xlabels)), label="From bytecode handler")
    axis.xaxis.tick_top()
    ...
    figure.colorbar(image, ax=axis, fraction=0.01, pad=0.01)
...
def parse_command_line():
    ...
    command_line_parser.add_argument(
        "--plot", "-p", action="store_true", help="plot dispatch pairs heatmap"
    )
    command_line_parser.add_argument(
        "--interactive",
        "-i",
    ...
    command_line_parser.add_argument(
        ...
        help=(
            "file to save the plot file to. File type is deduced from the "
            "extension. PDF, SVG, PNG supported"
        ),
    )
    command_line_parser.add_argument(
        "--sort-sources-relative",
        "-r",
        action="store_true",
        help=(
            "print top sources in order to how often they dispatch to the "
...
def main():
    ...
    if program_options.plot:
        ...
        print_top_dispatch_sources_and_destinations(
            dispatches_table,
            program_options.top_dispatches_for_bytecode,
            program_options.top_entries_count,
            program_options.sort_sources_relative,
        )
    else:
