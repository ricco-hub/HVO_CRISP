...
JIT_CODE_SYMBOLS_RE = re.compile(r"(LazyCompile|Compile|Eval|Script):(\*|~)")
GC_SYMBOLS_RE = re.compile(r"v8::internal::Heap::CollectGarbage")
...
def collapsed_callchains_generator(
    perf_stream,
    hide_other=False,
    hide_compiler=False,
    hide_jit=False,
    ...
    for line in perf_stream:
        ...
        if not line:
            if not skip_until_end_of_chain and current_chain and not hide_other:
        ...
        if symbol == "[unknown]" and current_chain and current_chain[-1] == "[unknown]":
            continue
...
def write_handlers_report(output_stream, callchains):
    ...
    for bytecode_name, count in handler_counters:
        output_stream.write(
            "{}\t{}\t{:.3f}%\n".format(
                bytecode_name, count, 100.0 * count / samples_num
            )
        )
...
def parse_command_line():
    ...
    command_line_parser.add_argument(
        "--hide-other", help="Hide other samples", action="store_true"
    )
    command_line_parser.add_argument(
        "--hide-compiler", help="Hide samples during compilation", action="store_true"
    )
    command_line_parser.add_argument(
        "--hide-jit", help="Hide samples from JIT code execution", action="store_true"
    )
    command_line_parser.add_argument(
        "--hide-gc", help="Hide samples from garbage collection", action="store_true"
    )
    command_line_parser.add_argument(
...
def main():
    ...
    perf = subprocess.Popen(
        ["perf", "script", "--fields", "ip,sym", "-i", program_options.perf_filename],
        stdout=subprocess.PIPE,
    )

    callchains = collapsed_callchains_generator(
        perf.stdout,
        program_options.hide_other,
        program_options.hide_compiler,
        program_options.hide_jit,
        program_options.hide_gc,
