...
class BenchmarkResult:
    ...
    def Compare(self, other):
        ...
        if abs(compare_num) > 0.1:
            ...
            z = Statistics.ComputeZ(
                other.result_, other.sigma_, self.result_, self.count_
            )
...
class Benchmark:
    ...
    def appendResult(self, run_name, trace):
        ...
        stddev = float(
            trace.get("stddev") or Statistics.StandardDeviation(values, mean)
        )
        units = trace["units"]
...
"""
    ...
    def StartSuite(self, suite_name, run_names):
        self.Print("<h2>")
        self.Print(
            '<a name="%s">%s</a> <a href="#top">(top)</a>' % (suite_name, suite_name)
        )
        self.Print("</h2>")
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="Compare perf trybot JSON files and "
        + "output the results into a pleasing HTML page."
    )
    parser.add_argument(
        "-t", "--title", dest="title", help="Optional title of the web page"
    )
    parser.add_argument(
        "-o",
        "--output",
