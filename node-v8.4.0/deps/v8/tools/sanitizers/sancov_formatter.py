...
SYMBOLIZER = os.path.join(
    BASE_DIR, "third_party", "llvm-build", "Release+Asserts", "bin", "llvm-symbolizer"
)
...
def write_instrumented(options):
    ...
    logging.info("Reading instrumented lines from %d executables.", len(exe_list))
    pool = Pool(CPUS)
    try:
    ...
    logging.info(
        "Read data from %d executables, which covers %d files.",
        len(data["tests"]),
        len(data["files"]),
    )
...
def merge(options):
    ...
    assert os.path.exists(options.coverage_dir) and os.path.isdir(options.coverage_dir)
    ...
    logging.info("Merging %d sancov files into %s", len(inputs), options.json_input)
...
def split(options):
    ...
    logging.info(
        "Splitting off %d coverage files from %s",
        len(data["files"]),
        options.json_input,
    )
...
def main(args=None):
    ...
    parser.add_argument("--coverage-dir", help="Path to the sancov output files.")
    parser.add_argument(
        "--json-input", help="Path to an existing json file with coverage data."
    )
    parser.add_argument("--json-output", help="Path to a file to write json output to.")
    parser.add_argument(
        "--output-dir", help="Directory where to put split output files to."
    )
    parser.add_argument(
        "action", choices=["all", "merge", "split"], help="Action to perform."
    )
