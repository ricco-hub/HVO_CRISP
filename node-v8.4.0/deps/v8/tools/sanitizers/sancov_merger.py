...
def generate_inputs(keep, coverage_dir, file_map, cpus):
    ...
    for executable, files in file_map.iteritems():
        ...
        # list of files.
        inputs.extend(
            [(keep, coverage_dir, executable, i, b) for i, b in enumerate(buckets)]
        )
    return inputs
...
def merge_test_runner_output(options):
    ...
    inputs = generate_inputs(options.keep, options.coverage_dir, file_map, CPUS)

    logging.info(
        "Executing %d merge jobs in parallel for %d executables."
    ...
    # file name.
    inputs = [
        (options.keep, options.coverage_dir, executable, None, files)
        for executable, files in file_map.iteritems()
    ]
    ...
    for f in os.listdir(options.swarming_output_dir):
        ...
        if match:
            if os.path.exists(os.path.join(options.coverage_dir, f)):
                ...
                os.rename(
                    os.path.join(options.swarming_output_dir, f),
                    os.path.join(options.coverage_dir, f),
                )

    inputs = [(options.swarming_output_dir, options.coverage_dir, f) for f in files]
...
def main():
    ...
    parser.add_argument(
        "--coverage-dir", required=True, help="Path to the sancov output files."
    )
    parser.add_argument(
        "--keep",
        default=False,
    ...
    parser.add_argument(
        "--swarming-output-dir", help="Folder containing a results shard from swarming."
    )
    ...
    if options.swarming_output_dir:
        ...
        assert os.path.exists(options.swarming_output_dir) and os.path.isdir(
            options.swarming_output_dir
        )
