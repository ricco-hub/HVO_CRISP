...
def GetFlags(lines, build_dir):
    ...
    for line in lines:
        ...
        cc_file = [
            x
            for x in command_line
            if x.endswith(".cc") or x.endswith(".c") or x.endswith(".cpp")
        ]
        if len(cc_file) != 1:
        ...
        warnings = [x for x in command_line if x.startswith("/wd" if is_win else "-W")]
        others = [
            x
            for x in command_line
            if x not in defines
            and x not in include_dirs
            and x not in dash_f
            and x not in warnings
            and x not in cc_file
        ]
...
def main():
    if len(sys.argv) < 4:
        print (
            "usage: %s gn_outdir gyp_outdir gn_target "
    ...
    all_gyp_flags = GetFlags(gyp.splitlines(), os.path.join(os.getcwd(), gyp_out_dir))
    all_gn_flags = GetFlags(gn.splitlines(), os.path.join(os.getcwd(), gn_out_dir))
    ...
    for filename in sorted(file_list):
        ...
        differences += CompareLists(
            gyp_flags, gn_flags, "include_dirs", ["-I%s" % os.path.dirname(BASE_DIR)]
        )
        differences += CompareLists(
            gyp_flags,
            gn_flags,
            ...
            dont_care_gn=["-Wendif-labels", "-Wextra", "-Wsign-compare",]
