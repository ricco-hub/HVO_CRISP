...
if action in ["help", "-h", "--help"] or len(sys.argv) != 3:
    print (
        "Usage: %s <action> <inputfile>, where action can be: \n"
...
def AddTransition(from_map, to_map, reason):
    ...
    if to_map in targets:
        ...
        print (
            "// warning: already have a transition from %s to %s, reason: %s"
...
with open(filename, "r") as f:
    last_to_map = ""
    for line in f:
        if not line.startswith("[TraceMaps: "):
            continue
        words = line.split(" ")
        event = words[1]
        if event == "InitialMap":
