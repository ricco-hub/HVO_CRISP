...
def SetEnvironmentAndGetRuntimeDllDirs():
  ...
  """
    ...
    if (
        sys.platform in ("win32", "cygwin") or os.path.exists(json_data_file)
...
def DetectVisualStudioPath():
  ...
  """
    ...
    if version_as_year not in year_to_version:
        raise Exception(
            (
                "Visual Studio version %s (from GYP_MSVS_VERSION)"
    ...
    raise Exception(
        ("Visual Studio Version %s (from GYP_MSVS_VERSION)" " not found.")
        % (version_as_year)
    )
...
def _CopyRuntimeImpl(target, source, verbose=True):
  ...
  """
    if os.path.isdir(os.path.dirname(target)) and (
        ...
        or os.stat(target).st_mtime != os.stat(source).st_mtime
    ):
        if verbose:
...
def _CopyRuntime(target_dir, source_dir, target_cpu, debug):
  ...
  directory does exist. Handles VS 2013 and VS 2015."""
    ...
    if not debug and os.environ.get("GYP_MSVS_OVERRIDE_PATH"):
        pgo_x86_runtime_dir = os.path.join(
            os.environ.get("GYP_MSVS_OVERRIDE_PATH"), "VC", "bin"
        )
        ...
        if target_cpu == "x86":
            ...
            if os.path.exists(source_x64):
                _CopyRuntimeImpl(os.path.join(target_dir, pgo_runtime_dll), source_x64)
        else:
