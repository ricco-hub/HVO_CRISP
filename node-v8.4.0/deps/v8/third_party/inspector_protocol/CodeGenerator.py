...
class Protocol(object):
    ...
    def read_protocol_file(self, file_name):
        ...
        version = (
            parsed_json["version"]["major"] + "." + parsed_json["version"]["minor"]
        )
        domains = []
    ...
    def create_type_definitions(self):
        imported_namespace = (
            "::".join(self.config.imported.namespace) if self.config.imported else ""
        )
        ...
        for domain in self.json_api["domains"]:
            ...
            for type in domain["types"]:
                ...
                if (
                    ...
                    self.type_definitions[type_name] = wrap_array_definition(
                        self.type_definitions[items_type]
                    )
    ...
    def is_async_command(self, domain, command):
        if not self.config.protocol.options:
            return False
        return self.check_options(
            self.config.protocol.options, domain, command, "async", None, False
        )
...
def main():
    ...
    for domain in protocol.json_api["domains"]:
        ...
        if domain["domain"] in protocol.generate_domains:
            outputs[
                os.path.join(
                    config.protocol.output, to_file_name(config, class_name + ".h")
                )
            ] = h_template.render(template_context)
            outputs[
                os.path.join(
                    config.protocol.output, to_file_name(config, class_name + ".cpp")
                )
            ] = cpp_template.render(template_context)
            if domain["domain"] in protocol.exported_domains:
                outputs[
                    os.path.join(
                        config.exported.output, to_file_name(config, class_name + ".h")
                    )
