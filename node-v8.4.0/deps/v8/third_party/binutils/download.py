...
DETECT_HOST_ARCH = os.path.abspath(
    os.path.join(BINUTILS_DIR, "../../gypfiles/detect_v8_host_arch.py")
)
...
def GetArch():
    gyp_host_arch = re.search("host_arch=(\S*)", os.environ.get("GYP_DEFINES", ""))
    if gyp_host_arch:
...
def FetchAndExtract(arch):
    ...
    if os.path.exists(stampfile):
        if (
            os.path.exists(tarball)
            and os.path.exists(outdir)
            and checksum == ReadFile(stampfile)
        ):
            return 0
        else:
            os.unlink(stampfile)
