...
injected_script_externs_file = path.join(
    v8_inspector_path, "injected_script_externs.js"
)
debugger_script_source_name = path.join(v8_inspector_path, "debugger-script.js")
debugger_script_externs_file = path.join(
    v8_inspector_path, "debugger_script_externs.js"
)

generate_protocol_externs.generate_protocol_externs(
    protocol_externs_file, path.join(v8_inspector_path, "js_protocol.json")
)

error_warning_regex = re.compile(r"WARNING|ERROR")

closure_compiler_jar = path.join(
    v8_inspector_path, "build", "closure-compiler", "closure-compiler.jar"
)

common_closure_args = ["--checks_only", "--warning_level", "VERBOSE"]
...
def find_java():
    ...
    if not is_ok:
        print "NOTE: Java executable version %d.%d or above not found in $PATH." % (
            java_required_major,
            java_required_minor,
        )
        sys.exit(0)
...
spawned_compiler_command = (
    java_exec + ["-jar", closure_compiler_jar] + common_closure_args
)
...
command = spawned_compiler_command + [
    "--externs",
    debugger_script_externs_file,
    "--js",
    debugger_script_source_name,
    "--new_type_inf",
]
...
injectedscript_check_script_path = path.join(
    v8_inspector_path, "build", "check_injected_script_source.py"
)
validate_injected_script_proc = popen(
    [sys.executable, injectedscript_check_script_path, injected_script_source_name]
)
