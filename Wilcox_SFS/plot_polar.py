...
main_plot = HVOPlot(
    "Polar Field Strength of the Sun", "Time (years)", "Polar Field (G)"
)

for idx, file_path in enumerate(pkl_files):
    label = Path(file_path).stem.replace("Polar_", "")  # e.g., "N"
    y_key = label
    ...
    main_plot.add_hover_tool(source, "decimal_year", y_key, "Year", f"{label} Pole")
