...
# --- Unified filtering callback for all sources ---
def update_all_sources(attr, old, new, date_range, data_dict, sources_dict):
...
# --- Reset button ---
def reset_all():
    date_range_picker.value = (min_date, max_date)
...
df = pd.read_pickle("data/Tilt_L_av.pkl")
df["date"] = df["decimal_year"].apply(decimal_year_to_YYYYMMDD)
...
data = {"Average (L)": df.copy()}
del data["Average (L)"]["date"]
source_data = {name: ColumnDataSource(data=series) for name, series in data.items()}

# Create a scatter plot
angle_plot = HVOPlot("Tilt Angle of Heliospheric Current Sheet", "Time (years)", "Angle (degrees)")
angle_plot.line_plot(source, "Average (L)", "decimal_year", "L_av")
angle_plot.add_hover_tool(source, "decimal_year", "L_av", "Year", "Angle")
scatter = angle_plot.line_plot(source, "Average (L)", "decimal_year", "L_av")
...
plots = {}
plots["Average (L)"] = {"scatter": scatter}
...
download_csv_button.js_on_click(
    CustomJS(
        args=dict(
            source=source_data,
            plots=plots,
            x_range=angle_plot.get_plot().x_range,
            y_range=angle_plot.get_plot().y_range,
        ),
        code=js_callbacks
        + """
        const x_start = x_range.start;
