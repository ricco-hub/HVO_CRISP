...
with open("/var/www/html/Proton_Flux/E.txt", "r") as infile:
...
# ssn range
ssnr = open("/var/www/html/Proton_Flux/data_PLOT/SSN_range.txt", "w")
with open("/var/www/html/Proton_Flux/SSN.txt", "r") as infile:
    ...
    for i in range(len(SSN_all)):
        ...
        if float(date[0]) <= float(sline[0]) <= float(date[1]):
            ssnr.write(
                "%8.3f   %8.3f   %8.3f  %8.3f"
                % (float(sline[0]), float(sline[1]), float(sline[2]), float(sline[3]))
            )
            ssnr.write("\n")
